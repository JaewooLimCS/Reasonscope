...
export async function POST(request: Request) {
  try {
    ...
    const result: MethodResult = await runMethodById(
      method as Method,
      question,
      previousAttempts,
    );
    console.log(
      `[retry] ${result.method} previousAttempts.length = ${previousAttempts.length} returning attempts.length = ${result.attempts?.length ?? 'undefined'}`,
    );
